...
class ChromeSync:
    """Chrome多窗口同步控制器"""import tkinter as tk
...
if __name__ == "__main__":
    ...
    def __init__(self):
        self.is_syncing = False
        self.is_sync = False  # 内部同步状态

        # 窗口管理
        self.master_window = None
        self.sync_windows = []  # 被控窗口列表
        self.popup_windows = []  # 弹出窗口列表

        # 调试端口映射
        self.debug_ports = {}  # window_num -> debug_port
        self.base_debug_port = 9222

        # 事件节流
        self.last_mouse_position = (0, 0)
        self.last_move_time = 0
        self.move_interval = 0.01  # 10ms
        self.mouse_threshold = 2  # 像素阈值
    ...
    def add_sync_window(self, hwnd: int, window_num: int = None):
        ...
        # 设置调试端口映射
        if window_num:
            self.debug_ports[window_num] = self.base_debug_port + window_num
    ...
    def start_sync(self):
        ...
        try:
            ...
            # 启动弹窗监控
            self.popup_monitor_thread = threading.Thread(target=self._monitor_popups, daemon=True)
            self.popup_monitor_thread.start()
            ...
            print(f"同步已启动 - 主控: {self.master_window}, 被控: {self.sync_windows}")

        except Exception as e:
            self.stop_sync()
            raise Exception(f"启动同步失败: {str(e)}")
    ...
    def _on_mouse_event(self, event):
        ...
        try:
            ...
            # 检查是否是主控窗口或其弹出窗口
            is_master = current_window == self.master_window
            is_popup = current_window in self.popup_windows

            if not (is_master or is_popup):
                return

            # 检查鼠标是否在窗口内
            if not self._mouse_in_current_window(current_window):
                return

            # 移动事件节流
            if isinstance(event, mouse.MoveEvent):
                if not self._should_process_move_event(event):
                    return

            # 同步到其他窗口
            self._sync_mouse_event_to_windows(event, current_window, is_master)
    ...
    def _on_keyboard_event(self, event):
        ...
        try:
            ...
            # 同步到其他窗口
            self._sync_keyboard_event_to_windows(event, current_window, is_master)

        except Exception as e:
            print(f"键盘事件处理错误: {str(e)}")

    def _mouse_in_current_window(self, window_hwnd: int) -> bool:
        """检查鼠标是否在当前窗口内"""
        try:
            x, y = mouse.get_position()
    ...
    def _should_process_move_event(self, event) -> bool:
        """判断是否应该处理移动事件（节流）"""
        current_time = time.time()
        ...
        if hasattr(event, 'x') and hasattr(event, 'y'):
            ...
            # 如果无法获取坐标，使用当前鼠标位置
            event_x, event_y = win32gui.GetCursorPos()

        # 距离节流
        distance = ((event_x - self.last_mouse_position[0]) ** 2 +
                   (event_y - self.last_mouse_position[1]) ** 2) ** 0.5
        if distance < self.mouse_threshold:
            return False

        self.last_move_time = current_time
        self.last_mouse_position = (event_x, event_y)
        return True

    def _sync_mouse_event_to_windows(self, event, source_window: int, is_master: bool):
        """同步鼠标事件到其他窗口"""
        print(f"开始同步鼠标事件，源窗口: {source_window}, 是否主控: {is_master}")
        print(f"同步窗口列表: {self.sync_windows}")

        for hwnd in self.sync_windows:
            try:
                target_hwnd = hwnd if is_master else self._find_matching_popup(hwnd, source_window)
                print(f"目标窗口: {target_hwnd}, 源窗口: {source_window}")
                if target_hwnd and target_hwnd != source_window:
                    self._send_mouse_event(target_hwnd, event)
                else:
                    print(f"跳过窗口 {hwnd}: target_hwnd={target_hwnd}, source_window={source_window}")
            except Exception as e:
                print(f"同步鼠标事件到窗口 {hwnd} 失败: {str(e)}")

    def _sync_keyboard_event_to_windows(self, event, source_window: int, is_master: bool):
        """同步键盘事件到其他窗口"""
        for hwnd in self.sync_windows:
            try:
                target_hwnd = hwnd if is_master else self._find_matching_popup(hwnd, source_window)
                if target_hwnd and target_hwnd != source_window:
                    self._send_keyboard_event(target_hwnd, event)
            except Exception as e:
                print(f"同步键盘事件到窗口 {hwnd} 失败: {str(e)}")

    def _send_mouse_event(self, hwnd: int, event):
        """发送鼠标事件到指定窗口"""
        try:
            # 验证窗口句柄是否有效
            if not win32gui.IsWindow(hwnd):
                print(f"跳过无效窗口句柄: {hwnd}")
                return

            print(f"向窗口 {hwnd} 发送鼠标事件")
            # 获取鼠标坐标，兼容不同的属性名
            if hasattr(event, 'x') and hasattr(event, 'y'):
                event_x, event_y = event.x, event.y
            elif hasattr(event, 'position'):
                ...
                # 如果无法获取坐标，使用当前鼠标位置
                event_x, event_y = win32gui.GetCursorPos()
                print(f"警告: 无法从事件获取坐标，使用当前鼠标位置: ({event_x}, {event_y})")

            if isinstance(event, mouse.ButtonEvent):
                msg = win32con.WM_LBUTTONDOWN if event.event_type == mouse.DOWN else win32con.WM_LBUTTONUP
                if event.button == mouse.RIGHT:
                    msg = win32con.WM_RBUTTONDOWN if event.event_type == mouse.DOWN else win32con.WM_RBUTTONUP
                elif event.button == mouse.MIDDLE:
                    msg = win32con.WM_MBUTTONDOWN if event.event_type == mouse.DOWN else win32con.WM_MBUTTONUP

                # 转换坐标到目标窗口
                target_x, target_y = self._convert_coordinates(event_x, event_y, hwnd)
                lparam = target_y << 16 | target_x

                print(f"原始坐标: ({event_x}, {event_y}), 目标坐标: ({target_x}, {target_y})")
                print(f"发送 PostMessage: hwnd={hwnd}, msg={msg}, lparam={lparam}")

                result = win32gui.PostMessage(hwnd, msg, 0, lparam)
                print(f"PostMessage 结果: {result}")

            elif isinstance(event, mouse.WheelEvent):
                # 滚轮事件处理
                msg = win32con.WM_MOUSEWHEEL
                delta = int(event.delta * 120)  # 标准滚轮增量
                wparam = delta << 16
                target_x, target_y = self._convert_coordinates(event_x, event_y, hwnd)
                lparam = target_y << 16 | target_x
            ...
            print(f"发送鼠标事件失败: {str(e)}")

    def _send_keyboard_event(self, hwnd: int, event):
        """发送键盘事件到指定窗口"""
        try:
            # 获取虚拟键码
            vk_code = self._get_virtual_key_code(event.name)
            if vk_code:
                if event.event_type == keyboard.KEY_DOWN:
            ...
            print(f"发送键盘事件失败: {str(e)}")

    def _convert_coordinates(self, x: int, y: int, target_hwnd: int) -> tuple:
        """转换坐标到目标窗口相对坐标"""
        try:
            # 获取窗口客户区
            target_rect = win32gui.GetClientRect(target_hwnd)
            window_rect = win32gui.GetWindowRect(target_hwnd)

            # 计算客户区相对于屏幕的位置
            client_point = win32gui.ClientToScreen(target_hwnd, (0, 0))

            # 转换为客户区坐标
            client_x = x - client_point[0]
            client_y = y - client_point[1]

            print(f"坐标转换: 屏幕({x},{y}) -> 客户区({client_x},{client_y})")

            # 确保坐标在有效范围内
            if client_x < 0 or client_y < 0 or client_x > target_rect[2] or client_y > target_rect[3]:
                print(f"警告: 坐标超出客户区范围 {target_rect}")
    ...
    def _find_matching_popup(self, main_hwnd: int, source_popup: int) -> Optional[int]:
        """查找匹配的弹出窗口"""
        try:
            # 获取主窗口的弹出窗口
            popups = self._get_chrome_popups(main_hwnd)
            if popups:
                return popups[0]  # 简单返回第一个
        except:
            pass
        return main_hwnd

    def _get_chrome_popups(self, chrome_hwnd: int) -> List[int]:
        """获取Chrome弹出窗口"""
        popups = []
        try:
            _, chrome_pid = win32process.GetWindowThreadProcessId(chrome_hwnd)
            ...
            def enum_callback(hwnd, _):
                if win32gui.IsWindowVisible(hwnd):
                    _, popup_pid = win32process.GetWindowThreadProcessId(hwnd)
                    if popup_pid == chrome_pid and hwnd != chrome_hwnd:
                        class_name = win32gui.GetClassName(hwnd)
    ...
    def _set_windows_topmost(self):
        """设置窗口置顶"""
        for hwnd in self.sync_windows + [self.master_window]:
            try:
                win32gui.SetWindowPos(hwnd, win32con.HWND_TOPMOST, 0, 0, 0, 0,
                                    win32con.SWP_NOMOVE | win32con.SWP_NOSIZE)
                win32gui.SetWindowPos(hwnd, win32con.HWND_NOTOPMOST, 0, 0, 0, 0,
        ...
        while self.is_sync:
            try:
                if self.master_window:
                    current_popups = self._get_chrome_popups(self.master_window)

                    # 检查新弹出窗口
                    new_popups = [p for p in current_popups if p not in self.popup_windows]
                    for popup in new_popups:
                        self.popup_windows.append(popup)
                        print(f"检测到新弹出窗口: {popup}")

                    # 清理已关闭的弹出窗口
                    self.popup_windows = [p for p in self.popup_windows
                                        if win32gui.IsWindow(p)]
